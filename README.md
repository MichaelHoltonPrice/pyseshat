...
```bash
git clone https://github.com/MichaelHoltonPrice/terbuka
cd terbuka
```

If desired, create and activate a virtual environment named seshat_env:

```bash
python -m venv seshat_env
Set-ExecutionPolicy Unrestricted -Scope Process
.\seshat_env\Scripts\activate
```
...
is, pyseshat provides a core set of tools that multiple analyses and research
...
```bash
python -m unittest tests/test_pyseshat.py
```
